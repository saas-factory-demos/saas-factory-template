...
export async function POST(req: Request): Promise<Response> {
  ...
  // A5 後 Products 已有 unitPrice / currency / status 欄位；只接受 published 商品
  const productAny = product as unknown as Record<string, unknown>;
  const status = productAny.status as string | undefined;
  if (status !== 'published') {
    return Response.json(
      { error: `商品 ${productId} 狀態為 ${status ?? 'unknown'}，無法購買` },
      { status: 409 },
    );
  }
  ...
  const productCurrency = (productAny.currency as string | undefined) ?? 'TWD';
  ...
  const order = await payload.create({
    ...
    data: {
      ...
      items: [
        {
          productId,
          sku: (productAny.sku as string) ?? (productAny.slug as string) ?? productId,
          title: (productAny.title as string) ?? '商品',
          unitPrice,
          quantity,
        },
      ] as unknown as { value: unknown },
      currency: productCurrency as 'TWD',
      subtotal,
      discountTotal: 0,
      shippingFee: 0,
      taxAmount: 0,
  ...
  // payment-core Money.currency 只接受 CurrencyCode union（TWD/USD/...）；
  // Product.currency 已被 Payload schema 限制在這個範圍，直接 cast 即可
  const charge = await provider.charge({
    orderId: orderNumber, // 給藍新 MerchantOrderNo
    tenantId,
    method: 'credit',
    amount: { amount: total, currency: productCurrency as 'TWD' },
