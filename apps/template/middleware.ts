 ...
 * 組合 CSP 字串。
 *
 * 注意 script-src 暫時保留 `'unsafe-inline'` 與 `'unsafe-eval'`：
 * - Next.js 15 + React Server Components 在 production 仍需要少量內聯 hydration script
 * - `'strict-dynamic'` + nonce 改造需把所有 third-party script tag 都改成 next/script
 *   並注入 nonce，列為 v1.2 任務（待 CSP-2 issue）。
 */
function buildCsp(reportEndpoint: string): string {
  ...
  const directives: Record<string, string[]> = {
    'default-src': ["'self'"],
    'script-src': ["'self'", "'unsafe-inline'", "'unsafe-eval'", ...ANALYTICS_SCRIPT_SRC],
...
export function middleware(request: NextRequest): NextResponse {
  ...
  const csp = buildCsp(reportEndpoint);
