 ...
 *
 * 為何 hardcode 對照表：策展 preset 是 20 個有限枚舉，逐一給精準的英文氛圍詞，
 * 比起把中文 preset 名硬塞進 prompt 更能讓 model 抓對調性。未列出的（含
 * ui-ux-pro-max 擴充的 161 個）走 FALLBACK_MOOD 通用詞，由 industry 補充氛圍。
 *
 * 為何 Partial：擴充的 161 個 preset 不需逐一寫 mood（會變成 181 行手刻表）；
 * fallback 路徑已足夠表達通用視覺語言。
 */
const PRESET_MOOD: Readonly<Partial<Record<PresetKey, string[]>>> = Object.freeze({
