...
for (const site of DEMO_SITES) {
  test.describe(`demo site: ${site.label}`, () => {
    ...
    test('首頁 axe-core a11y scan（WCAG 2.1 AA）無 violation', async ({ page }) => {
      await page.goto(baseUrl!, { waitUntil: 'load' });
      // axe-core 用業界標準 ruleset（wcag2a, wcag2aa, wcag21a, wcag21aa, best-practice）
      // 範圍限定 body（排除 Next.js 注入的 dev tools 元素），impact >= moderate 才視為失敗
      ...
      expect(
        blocking,
        `axe-core 偵測到 serious/critical 違規：\n${summary}`,
      ).toEqual([]);
    });
  });
}
